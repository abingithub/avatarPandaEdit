...
#define RR_END_ALL_CODE_RECORD_REQUEST 13
#define RR_END_CODE_RECORD_REQUEST 12
#define RR_END_RECORD_REQUEST 11
...
volatile sig_atomic_t rr_record_code_requested = 0;
volatile sig_atomic_t rr_replay_requested = 0;
volatile sig_atomic_t rr_end_record_requested = 0;
volatile sig_atomic_t rr_end_all_code_records_requested = 0;
...
void qmp_begin_code_records(Error** errp)
{
    rr_record_code_requested = RR_CODE_RECORD_ALL_REQUEST;
}

void qmp_trace_one(const char* record_name, const char* start_addr,
		const char* end_addr, Error** errp) {
	rr_record_code_requested = RR_CODE_RECORD_TRACE_ONE_REQUEST;
    if(rr_requested_name)
    	g_free(rr_requested_name);
...
void qmp_begin_code_record(const char* record_name, Error** errp)
{
	if(record_name == NULL)
		return;
	rr_record_code_requested = RR_CODE_RECORD_ONE_REQUEST;
    if(rr_requested_name)
    	g_free(rr_requested_name);
    rr_requested_name = g_strdup(record_name);
}
...
{
    qmp_stop(NULL);
    rr_end_record_requested = RR_END_RECORD_REQUEST;
}

void qmp_end_code_record(const char* record_name, Error** errp)
{
	if(record_name == NULL)
		return;
	rr_end_record_requested = RR_END_CODE_RECORD_REQUEST;
    if(rr_requested_name)
    	g_free(rr_requested_name);
	rr_requested_name = g_strdup(record_name);
}

void qmp_end_all_code_records(Error** errp)
{
	rr_end_record_requested = RR_END_ALL_CODE_RECORD_REQUEST;
}

void qmp_list_stopped_code_recordings(Error** errp)
{
}
...
void hmp_trace_one(Monitor* mon, const QDict* qdict)
{
    ...
    const char* start_addr = qdict_get_try_str(qdict, "start_addr");
...
{
    Error* err;
    const char* record_name = qdict_get_try_str(qdict, "record_name");
    qmp_end_code_record(record_name, &err);
}

void hmp_end_all_code_records(Monitor* mon, const QDict* qdict)
{
    Error* err;
    qmp_end_all_code_records(&err);
}

static void monitor_printf_key(void *key, void *value, void *user_data){
	Monitor* mon = (Monitor*)user_data;
	monitor_printf(mon, "  %s\n", (const char*)key);
}

void hmp_list_stopped_code_recordings(Monitor* mon, const QDict* qdict)
{
	monitor_printf(mon, "The stopped code recordings:\n");
	if(rr_end_code_record_names){
		g_hash_table_foreach(rr_end_code_record_names, monitor_printf_key, NULL);
	}
    Error* err;
    qmp_list_stopped_code_recordings(&err);
}
...
int rr_do_begin_code_record(CPUState* cpu_state)
{
#ifdef CONFIG_SOFTMMU
	...
	rr_end_all_code_records_requested = 0;///////ignored/////////////////
	rr_mode = RR_CODE_RECORD;
...
int rr_do_begin_record(const char* file_name_full, CPUState* cpu_state)
{
	...
	if (rr_record_requested == RR_RECORD_REQUEST
			|| rr_record_requested == RR_RECORD_FROM_REQUEST) {
		// Force running state
		global_state_store_running();
...
{
#ifdef CONFIG_SOFTMMU
	if ((rr_end_record_requested == RR_END_RECORD_REQUEST)
			&& rr_in_record()) {
		...
		vm_start();
	} else if ((rr_end_record_requested == RR_END_CODE_RECORD_REQUEST)
			&& rr_in_code_record()) {
		if(rr_end_code_record_names == NULL)
			rr_end_code_record_names = g_hash_table_new_full(g_str_hash, g_str_equal,
					g_free, NULL);
		if(rr_requested_name){
			g_hash_table_insert(rr_end_code_record_names, g_strdup(rr_requested_name), NULL);
			if(rr_begin_code_record_names){
				if(g_hash_table_contains(rr_begin_code_record_names, rr_requested_name)){
	...
	} else if ((rr_end_record_requested == RR_END_ALL_CODE_RECORD_REQUEST)
			&& rr_in_code_record()) {
		NewTrace = 1;
		TraceDetail = 0;
		StartMemAddr = 0;
	    ...
	    rr_end_all_code_records_requested = 1;
	    if(rr_begin_code_record_names)
	    	g_hash_table_destroy(rr_begin_code_record_names);
	    if(rr_end_code_record_names)
	    	g_hash_table_destroy(rr_end_code_record_names);
	    rr_begin_code_record_names = NULL;
	    rr_end_code_record_names = NULL;
	    // turn off logging
	    rr_mode = RR_OFF;
		rr_reset_state(first_cpu);
		vm_start();
