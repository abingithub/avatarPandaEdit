...
enum {
	OP_ADDL, OP_ORL, OP_ADCL, OP_SBBL, OP_ANDL, OP_SUBL, OP_XORL, OP_CMPL,
};

/* i386 shift ops */
enum {
	OP_ROL, OP_ROR, OP_RCL, OP_RCR, OP_SHL, OP_SHR, OP_SHL1, /* undocumented */
	OP_SAR = 7,
};

enum {
	JCC_O, JCC_B, JCC_Z, JCC_BE, JCC_S, JCC_P, JCC_L, JCC_LE,
};

enum {
	/* I386 int registers */
	OR_EAX, /* MUST be even numbered */
	OR_ECX, OR_EDX, OR_EBX, OR_ESP, OR_EBP, OR_ESI, OR_EDI,

	OR_TMP0 = 16, /* temporary operand register */
	OR_TMP1, OR_A0, /* temporary register used when doing address evaluation */
};

enum {
	USES_CC_DST = 1, USES_CC_SRC = 2, USES_CC_SRC2 = 4, USES_CC_SRCT = 8,
};

/* Bit set if the global variable is live after setting CC_OP to X.  */
static const uint8_t cc_op_live[CC_OP_NB] = { [CC_OP_DYNAMIC] = USES_CC_DST
		| USES_CC_SRC | USES_CC_SRC2, [CC_OP_EFLAGS] = USES_CC_SRC,
		...
		[CC_OP_BMILGB ... CC_OP_BMILGQ] = USES_CC_DST | USES_CC_SRC, [CC_OP_ADCX
				] = USES_CC_DST | USES_CC_SRC, [CC_OP_ADOX] = USES_CC_SRC
				| USES_CC_SRC2, [CC_OP_ADCOX] = USES_CC_DST | USES_CC_SRC
				| USES_CC_SRC2, [CC_OP_CLR] = 0, [CC_OP_POPCNT] = USES_CC_SRC, };
...
static void gen_lea_v_seg(DisasContext *s, TCGMemOp aflag, TCGv a0, int def_seg,
		int ovr_seg) {
	switch (aflag) {
#ifdef TARGET_X86_64
	case MO_64:
...
static void gen_check_io(DisasContext *s, TCGMemOp ot, target_ulong cur_eip,
	...
	if (s->flags & HF_SVMI_MASK) {
		...
		tcg_gen_trunc_tl_i32(cpu_tmp2_i32, cpu_T0);
		gen_helper_svm_check_io(cpu_env, cpu_tmp2_i32, tcg_const_i32(svm_flags),
...
static CCPrepare gen_prepare_eflags_c(DisasContext *s, TCGv reg) {
	...
	case CC_OP_ADDB ... CC_OP_ADDQ:
		...
		add_sub: return (CCPrepare ) { .cond = TCG_COND_LTU, .reg = t0, .reg2 =
						t1, .mask = -1, .use_reg2 = true } ;
			...
			case CC_OP_POPCNT:
				return (CCPrepare ) { .cond = TCG_COND_NEVER, .mask = -1 } ;
					...
					case CC_OP_DECB ... CC_OP_DECQ:
						return (CCPrepare ) { .cond = TCG_COND_NE, .reg =
										cpu_cc_src, .mask = -1, .no_setcond =
								true } ;
							...
							case CC_OP_SHLB ... CC_OP_SHLQ:
								...
								return (CCPrepare ) { .cond = TCG_COND_NE,
									...
									case CC_OP_MULB ... CC_OP_MULQ:
										return (CCPrepare ) { .cond =
														TCG_COND_NE, .reg =
														cpu_cc_src, .mask = -1 } ;

											case CC_OP_BMILGB ... CC_OP_BMILGQ:
												size = s->cc_op - CC_OP_BMILGB;
												t0 = gen_ext_tl(reg, cpu_cc_src,
														size, false);
												return (CCPrepare ) { .cond =
																TCG_COND_EQ,
																.reg = t0,
															...
															case CC_OP_SARB
																...
																return (CCPrepare ) {
																	...
																	default:
																		...
																		static CCPrepare gen_prepare_eflags_p(
																				DisasContext *s,
																				TCGv reg) {
																			gen_compute_eflags(
																					s);
																			return (CCPrepare ) {
																				...
																				static CCPrepare gen_prepare_eflags_s(
																						DisasContext *s,
																						TCGv reg) {
																					switch (s->cc_op) {
																					case CC_OP_DYNAMIC:
																							...
																							case CC_OP_POPCNT:
																								return (CCPrepare ) {
																									...
																									default: {
																										...
																										static CCPrepare gen_prepare_eflags_o(
																												DisasContext *s,
																												TCGv reg) {
																											switch (s->cc_op) {
																											case CC_OP_ADOX:
																													...
																													case CC_OP_POPCNT:
																														return (CCPrepare ) {
																															...
																															default:
																																...
																																static CCPrepare gen_prepare_eflags_z(
																																		DisasContext *s,
																																		TCGv reg) {
																																	switch (s->cc_op) {
																																	case CC_OP_DYNAMIC:
																																					...
																																					case CC_OP_POPCNT:
																																						return (CCPrepare ) {
																																							...
																																							default: {
																																								...
																																								static CCPrepare gen_prepare_cc(
																																										DisasContext *s,
																																										int b,
																																										TCGv reg) {
																																									int
																																								...
																																								static inline void gen_compute_eflags_c(
																																										DisasContext *s,
																																										TCGv reg) {
																																									gen_setcc1(
																																											s,
																																								...
																																								/* generate a conditional jump to label 'l1' according to jump opcode
																																								 value 'b'. In the fast case, T0 is guaranted not to be used. */
																																								static inline void gen_jcc1_noeob(
																																										DisasContext *s,
																																										int b,
																																										TCGLabel *l1) {
																																									CCPrepare cc =
																																								...
																																								/* XXX: does not work with gdbstub "ice" single step - not a
																																								 serious problem */
																																								static TCGLabel *gen_jz_ecx_string(
																																										DisasContext *s,
																																										target_ulong next_eip) {
																																									TCGLabel *l1 =
																																											gen_new_label();
...
{                                                                             \
    ...
    gen_jmp(s, cur_eip);                                                      \
																																								...
																																								static void gen_helper_fp_arith_ST0_FT0(
																																										int op) {
																																									switch (op) {
																																									case 0:
																																								...
																																								static void gen_helper_fp_arith_STN_ST0(
																																										int op,
																																										int opreg) {
																																									TCGv_i32 tmp =
																																											tcg_const_i32(
																																									...
																																									switch (op) {
																																									case 0:
																																										gen_helper_fadd_STN_ST0(
																																												cpu_env,
																																												tmp);
																																										break;
																																									case 1:
																																										gen_helper_fmul_STN_ST0(
																																												cpu_env,
																																												tmp);
																																										break;
																																									case 4:
																																										gen_helper_fsubr_STN_ST0(
																																												cpu_env,
																																												tmp);
																																										break;
																																									case 5:
																																										gen_helper_fsub_STN_ST0(
																																												cpu_env,
																																												tmp);
																																										break;
																																									case 6:
																																								...
																																								static AddressParts gen_lea_modrm_0(
																																										CPUX86State *env,
																																										DisasContext *s,
																																										int modrm) {
																																									int
																																									...
																																									default:
																																										...
																																										static void gen_nop_modrm(
																																												CPUX86State *env,
																																												DisasContext *s,
																																												int modrm) {
																																											(void) gen_lea_modrm_0(
																																													env,
																																													s,
																																													modrm);
																																										}
																																										...
																																										static inline int insn_const_size(
																																												TCGMemOp ot) {
																																											if (ot
																																													<= MO_32) {
																																												return 1
																																										...
																																										static inline bool use_goto_tb(
																																												DisasContext *s,
																																												target_ulong pc) {
																																										...
																																										static inline void gen_goto_tb(
																																												DisasContext *s,
																																												int tb_num,
																																												target_ulong eip) {
																																											target_ulong pc =
																																										...
																																										static inline void gen_op_movl_T0_seg(
																																												int seg_reg) {
																																											tcg_gen_ld32u_tl(
																																													cpu_T0,
																																													cpu_env,
																																													offsetof(
																																															CPUX86State,
																																															segs[seg_reg].selector));
																																										}

																																										static inline void gen_op_movl_seg_T0_vm(
																																												int seg_reg) {
																																											tcg_gen_ext16u_tl(
																																													cpu_T0,
																																													cpu_T0);
																																										...
																																										static inline void gen_svm_check_intercept_param(
																																												DisasContext *s,
																																												target_ulong pc_start,
																																												uint32_t type,
																																												uint64_t param) {
																																										...
																																										static inline void gen_svm_check_intercept(
																																												DisasContext *s,
																																												target_ulong pc_start,
																																												uint64_t type) {
																																											gen_svm_check_intercept_param(
																																													s,
																																													pc_start,
																																													type,
																																													0);
																																										...
																																										static inline void gen_stack_update(
																																												DisasContext *s,
																																												int addend) {
																																											gen_op_add_reg_im(
																																													mo_stacksize(
																																										...
																																										static inline void gen_stack_A0(
																																												DisasContext *s) {
																																											gen_lea_v_seg(
																																													s,
																																													s->ss32 ?
																																										...
																																										static void gen_unknown_opcode(
																																												CPUX86State *env,
																																												DisasContext *s) {
																																											gen_illegal_opcode(
																																													s);
																																										...
																																										/* End of block.
																																										 If INHIBIT, set HF_INHIBIT_IRQ_MASK if it isn't already set.  */
																																										static void gen_eob_inhibit_irq(
																																												DisasContext *s,
																																												bool inhibit) {
																																											gen_eob_worker(
																																													s,
																																										...
																																										static inline void gen_ldq_env_A0(
																																												DisasContext *s,
																																												int offset) {
																																											tcg_gen_qemu_ld_i64(
																																													cpu_tmp1_i64,
																																										...
																																										static inline void gen_stq_env_A0(
																																												DisasContext *s,
																																												int offset) {
																																											tcg_gen_ld_i64(
																																													cpu_tmp1_i64,
																																										...
																																										static inline void gen_ldo_env_A0(
																																												DisasContext *s,
																																												int offset) {
																																											int mem_index =
																																													s->mem_index;
																																										...
																																										static inline void gen_sto_env_A0(
																																												DisasContext *s,
																																												int offset) {
																																											int mem_index =
																																													s->mem_index;
																																										...
																																										static inline void gen_op_movo(
																																												int d_offset,
																																												int s_offset) {
																																											tcg_gen_ld_i64(
																																													cpu_tmp1_i64,
																																										...
																																										static inline void gen_op_movq(
																																												int d_offset,
																																												int s_offset) {
																																											tcg_gen_ld_i64(
																																													cpu_tmp1_i64,
																																										...
																																										static inline void gen_op_movl(
																																												int d_offset,
																																												int s_offset) {
																																											tcg_gen_ld_i32(
																																													cpu_tmp2_i32,
																																										...
																																										static inline void gen_op_movq_env_0(
																																												int d_offset) {
																																											tcg_gen_movi_i64(
																																													cpu_tmp1_i64,
																																													0);
																																										...
																																										typedef void (*SSEFunc_i_ep)(
																																												TCGv_i32 val,
																																												TCGv_ptr env,
																																												TCGv_ptr reg);
																																										typedef void (*SSEFunc_l_ep)(
																																												TCGv_i64 val,
																																												TCGv_ptr env,
																																												TCGv_ptr reg);
																																										typedef void (*SSEFunc_0_epi)(
																																												TCGv_ptr env,
																																												TCGv_ptr reg,
																																												TCGv_i32 val);
																																										typedef void (*SSEFunc_0_epl)(
																																												TCGv_ptr env,
																																												TCGv_ptr reg,
																																												TCGv_i64 val);
																																										typedef void (*SSEFunc_0_epp)(
																																												TCGv_ptr env,
																																												TCGv_ptr reg_a,
																																												TCGv_ptr reg_b);
																																										typedef void (*SSEFunc_0_eppi)(
																																												TCGv_ptr env,
																																												TCGv_ptr reg_a,
																																												TCGv_ptr reg_b,
																																												TCGv_i32 val);
																																										typedef void (*SSEFunc_0_ppi)(
																																												TCGv_ptr reg_a,
																																												TCGv_ptr reg_b,
																																												TCGv_i32 val);
																																										typedef void (*SSEFunc_0_eppt)(
																																												TCGv_ptr env,
																																												TCGv_ptr reg_a,
																																												TCGv_ptr reg_b,
																																												TCGv val);
...
#define SSE_FOP(x) { gen_helper_ ## x ## ps, gen_helper_ ## x ## pd, \
                     gen_helper_ ## x ## ss, gen_helper_ ## x ## sd, }
																																										...
																																										static const SSEFunc_0_epp sse_op_table1[256][4] =
																																												{
																																														...
																																														[0x10
																																																		...
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* movups, movupd, movss, movsd */
																																														[0x11
																																																] = {
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																														...
																																														[0x12
																																																		...
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* movlps, movlpd, movsldup, movddup */
																																														[0x13
																																																] = {
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* movlps, movlpd */
																																														[0x14
																																																] = {
																																																		gen_helper_punpckldq_xmm,
																																														...
																																														[0x16
																																																		...
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* movhps, movhpd, movshdup */
																																														[0x17
																																																] = {
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* movhps, movhpd */
																																														...
																																														[0x2a
																																																		...
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* cvtpi2ps, cvtpi2pd, cvtsi2ss, cvtsi2sd */
																																														[0x2b
																																																] = {
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* movntps, movntpd, movntss, movntsd */
																																														[0x2c
																																																] = {
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* cvttps2pi, cvttpd2pi, cvttsd2si, cvttss2si */
																																														[0x2d
																																																] = {
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* cvtps2pi, cvtpd2pi, cvtsd2si, cvtss2si */
																																														[0x2e
																																																] = {
																																																		gen_helper_ucomiss,
																																																		gen_helper_ucomisd },
																																														...
																																														[0x54
																																																		...
																																																		gen_helper_pand_xmm,
																																																		gen_helper_pand_xmm }, /* andps, andpd */
																																														[0x55
																																																] = {
																																																		gen_helper_pandn_xmm,
																																																		gen_helper_pandn_xmm }, /* andnps, andnpd */
																																														[0x56
																																																] = {
																																																		gen_helper_por_xmm,
																																																		gen_helper_por_xmm }, /* orps, orpd */
																																														[0x57
																																																] = {
																																																		gen_helper_pxor_xmm,
																																																		gen_helper_pxor_xmm }, /* xorps, xorpd */
																																														[0x58
																																																] =
																																																		SSE_FOP(
																																																				add),
																																														...
																																														[0x6e
																																																		...
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* movd mm, ea */
																																														[0x6f
																																																] = {
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* movq, movdqa, , movqdu */
																																														[0x70
																																																] = {
																																														...
																																														[0x7e
																																																		...
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* movd, movd, , movq */
																																														[0x7f
																																																] = {
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* movq, movdqa, movdqu */
																																														[0xc4
																																																] = {
																																																		SSE_SPECIAL,
																																																		SSE_SPECIAL }, /* pinsrw */
																																										...
																																										static const SSEFunc_0_epp sse_op_table2[3
																																												* 8][2] =
																																												{
																																														[0
																																																+ 2] = MMX_OP2(psrlw),
...
static target_ulong disas_insn(CPUX86State *env, DisasContext *s,
	...
	case 0xc4: /* 3-byte VEX */
		...
		if (s->code32 && !s->vm86) {
			static const int pp_prefix[4] = { 0, PREFIX_DATA, PREFIX_REPZ,
			PREFIX_REPNZ };
			...
			if (prefixes
					& (PREFIX_REPZ | PREFIX_REPNZ | PREFIX_LOCK | PREFIX_DATA)) {
				goto illegal_op;
			}
#ifdef TARGET_X86_64
			if (x86_64_hregs) {
	...
	case 0xf7:
		...
		case 4: /* mul */
			...
			case MO_32:
				...
				tcg_gen_mulu2_i32(cpu_tmp2_i32, cpu_tmp3_i32, cpu_tmp2_i32,
						cpu_tmp3_i32);
		...
		case 5: /* imul */
			...
			case MO_32:
				tcg_gen_trunc_tl_i32(cpu_tmp2_i32, cpu_T0);
				tcg_gen_trunc_tl_i32(cpu_tmp3_i32, cpu_regs[R_EAX]);
				tcg_gen_muls2_i32(cpu_tmp2_i32, cpu_tmp3_i32, cpu_tmp2_i32,
						cpu_tmp3_i32);
				tcg_gen_extu_i32_tl(cpu_regs[R_EAX], cpu_tmp2_i32);
				tcg_gen_extu_i32_tl(cpu_regs[R_EDX], cpu_tmp3_i32);
	...
	case 0x1b1: /* cmpxchg Ev, Gv */
	{
		...
		if (s->prefix & PREFIX_LOCK) {
			...
			gen_lea_modrm(env, s, modrm);
			tcg_gen_atomic_cmpxchg_tl(oldv, cpu_A0, cmpv, newv, s->mem_index,
					ot | MO_LE);
	...
	case 0x87: /* xchg Ev, Gv */
		...
		if (mod == 3) {
			rm = (modrm & 7) | REX_B(s);
			do_xchg_reg: gen_op_mov_v_reg(ot, cpu_T0, reg);
			gen_op_mov_v_reg(ot, cpu_T1, rm);
			gen_op_mov_reg_v(ot, rm, cpu_T0);
			gen_op_mov_reg_v(ot, reg, cpu_T1);
		} else {
			...
			tcg_gen_atomic_xchg_tl(cpu_T1, cpu_A0, cpu_T0, s->mem_index,
					ot | MO_LE);
			gen_op_mov_reg_v(ot, reg, cpu_T1);
		}
		break;
	...
	case 0x1b5: /* lgs Gv */
		op = R_GS;
		do_lxx: ot = dflag != MO_16 ? MO_32 : MO_16;
		modrm = cpu_ldub_code(env, s->pc++);
		reg = ((modrm >> 3) & 7) | rex_r;
		mod = (modrm >> 6) & 3;
		if (mod == 3)
	...
	case 0xd8 ... 0xdf:
		...
		if (mod != 3) {
			...
			case 0x30 ... 0x37: /* fixxx */
			{
				...
				switch (op >> 4) {
				case 0:
					tcg_gen_qemu_ld_i32(cpu_tmp2_i32, cpu_A0, s->mem_index,
							MO_LEUL);
			...
			case 0x38 ... 0x3b: /* filds, fisttps, fists, fistps */
				...
				case 1:
					...
					case 1:
						...
						tcg_gen_qemu_st_i32(cpu_tmp2_i32, cpu_A0, s->mem_index,
								MO_LEUL);
						break;
					case 2:
			...
			case 0x18 ... 0x1b: {
				int op1;
				TCGLabel *l1;
				static const uint8_t fcmov_cc[8] = { (JCC_B << 1), (JCC_Z << 1),
						(JCC_BE << 1), (JCC_P << 1), };
	...
	case 0x1bb: /* btc */
		...
		default:
			...
			gen_compute_eflags(s);
			tcg_gen_deposit_tl(cpu_cc_src, cpu_cc_src, cpu_tmp4, ctz32(CC_C),
					1);
			break;
		}
		break;
	...
	case 0x9b: /* fwait */
		...
		break;
	case 0xcc: /* int3 */
		gen_interrupt(s, EXCP03_INT3, pc_start - s->cs_base,
				s->pc - s->cs_base);
		break;
	case 0xcd: /* int N */
		val = cpu_ldub_code(env, s->pc++);
...
:		/* sgdt */
	...
	return s->pc;
	illegal_op: gen_illegal_opcode(s);
	return s->pc;
	unknown_op: gen_unknown_opcode(env, s);
	return s->pc;
}
...
void tcg_x86_init(void) {
	...
	static const char seg_base_names[6][8] = { [R_CS] = "cs_base", [R_DS
			] = "ds_base", [R_ES] = "es_base", [R_FS] = "fs_base", [R_GS
			] = "gs_base", [R_SS] = "ss_base", };
	...
	cpu_cc_op = tcg_global_mem_new_i32(cpu_env, offsetof(CPUX86State, cc_op),
			"cc_op");
	...
	for (i = 0; i < CPU_NB_REGS; ++i) {
		cpu_regs[i] = tcg_global_mem_new(cpu_env,
				offsetof(CPUX86State, regs[i]), reg_names[i]);
	}

	for (i = 0; i < 6; ++i) {
		cpu_seg_base[i] = tcg_global_mem_new(cpu_env,
				offsetof(CPUX86State, segs[i].base), seg_base_names[i]);
	}

	for (i = 0; i < 4; ++i) {
		cpu_bndl[i] = tcg_global_mem_new_i64(cpu_env,
				offsetof(CPUX86State, bnd_regs[i].lb), bnd_regl_names[i]);
		cpu_bndu[i] = tcg_global_mem_new_i64(cpu_env,
				offsetof(CPUX86State, bnd_regs[i].ub), bnd_regu_names[i]);
	}
}
...
void gen_intermediate_code(CPUX86State *env, TranslationBlock *tb) {
	...
	for (;;) {
		...
		if (unlikely(
				cpu_breakpoint_test(cs, pc_ptr, tb->flags & HF_RF_MASK ? BP_GDB : BP_ANY))) {
		...
		if (unlikely(
				panda_callbacks_insn_translate(ENV_GET_CPU(env), pc_ptr))) {
		 ...
		 */
		if (((tb->cflags & CF_USE_ICOUNT) || rr_in_record()
				|| rr_in_code_record() || rr_in_replay())
				&& ((pc_ptr & TARGET_PAGE_MASK)
						!= ((pc_ptr + TARGET_MAX_INSN_SIZE - 1)
								& TARGET_PAGE_MASK)
		...
		if (tcg_op_buf_full() || (pc_ptr - pc_start) >= (TARGET_PAGE_SIZE - 32)
				|| num_insns >= max_insns) {
			gen_jmp_im(pc_ptr - dc->cs_base);
			gen_eob(dc);
			break;
	...
	if (tb->cflags & CF_LAST_IO)
		gen_io_end();
	done_generating: gen_tb_end(tb, num_insns);
